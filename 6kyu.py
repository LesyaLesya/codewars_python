...
def order(sentence):
    # if (not len(sentence)) or (not any([i for i in sentence if i.isdigit()])):
    #     return ""
    # else:
    #     l = sentence.split()
    #     res = ''
    ...
    #                 res += j + ' '
    #     return res.strip()
    return ' '.join(sorted(sentence.split(), key=lambda i: sorted(i)))
...
Define another instance method called state that will return "The lamp is on." if it's on 
and "The lamp is off." otherwise.
"""

print('*** The Lamp: Revisited ***')
